    ...
    function liveDataInit(data) {
        ...
        if (config.flyouts) {
            nhmc.tooltips.render = function() {
                ...
                var flyout = $('<div id="tooltip"></div>').appendTo('body');
                ...
                if (areaResults.precincts[0] != 0) {
                    flyoutText.push('<table id="tooltip_results"><thead><tr><td class="tooltip_results_candidate">Candidate</td><td class="tooltip_results_votes">Votes</td></tr></thead><tbody>');
                    for (var i = 0, length = areaResults.data.length; i < length; i++) {
                        ...
                        flyoutText.push('<tr><td>' + candidateLastName + '</td>');
                        flyoutText.push('<td>' + formatThousands(candidateResult[1]) + '</td></tr>');
                    }
                    flyoutText.push('</tbody></table>');
