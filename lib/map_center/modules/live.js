...
$(document).one('coreInitialized', function() {
    ...
    $('#view_tabs').delegate('.view_tab_option:not(#view_tab_more_shown, #view_tab_options_more_shown)', 'click', function() {
        ...
        switchActiveSecondaryNav(mapValue);
    });
    
    function switchActiveSecondaryNav(mapView) {
        $('#lower_secondary_nav a').removeClass('active');
        $('#lower_secondary_nav a[href="#' + mapView + '"]')
            .addClass('active');
    }
    $('#lower_secondary_nav a').click(function() {
        var thisMapView = $(this).attr('href').substring(1);
        $('.view_tab_option[href="#' + thisMapView + '"]')
            .last().click();
        switchActiveSecondaryNav(thisMapView);
        return false;
    });
    ...
    function parseHash() {
        ...
        if (typeof(hashMapView) != 'undefined' && currentMapView != hashMapView) {
            ...
            if (selectedOption.length != 0) {
                ...
                switchActiveSecondaryNav(currentMapView);
            }
        } else {
            sidebarInit();
            getMapData(currentMapView);
