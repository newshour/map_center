...
nhmc.mapSpecificInit = function() {
    ...
    function liveDataInit(data) {
        ...
        // FIXME: Consider trying to reorder this list in the zero-results case.
        //  Non-major candidates without photos (or name recognition) might end
        //  up in the top positions just because of the randomness of the sort.
        for (var i = 0, length = data.breakdown.length; i < length; i++) {
            ...
            legendEntry.children('.candidate_vote_count').text(data.breakdown[i][1]);
