...
$(document).one('coreInitialized', function() {
    ...
    function liveDataInit(data) {
        ...
        for (var i = 0, length = data.breakdown.length; i < length; i++) {
            ...
            // Not ideal, since this is way too specific to the Maine caucus
            //  results from February and how we quickly implemented the map
            //  without counties (since results were not reported at that
            //  level). We just need to make sure we're looking at the correct
            //  result here.
            var stateShowing = $('#map_view').val().toUpperCase();
            var winnerId = "";
            // Basically, I screwed up but would rather not tamper with the 
            //  live results file from Maine now that it's been created. When I
            //  generated it, I changed all references to "Maine" to say 
            //  "Maine (statewide)" (equal to nhmc.config.USPSToState
            //  [stateShowing]) except the one in data.winners. (Always seem to
            //  forget _something_.) If we run into this kind of situation 
            //  elsewhere, that file will be generated properly--but that means 
            //  that on the off-chance we're looking at such a map, we need to 
            //  check both possible keys under data.winners to see if anything's 
            //  defined there.
            if (stateShowing.indexOf('_EMPTY') != -1) {
                stateShowing = stateShowing.substring(0, stateShowing.indexOf('_EMPTY'));
                if (data.winners[nhmc.config.USPSToState[stateShowing]]) {
                    winnerId = data.winners[nhmc.config.USPSToState[stateShowing]];
                } else {
                    stateShowing = $('#map_view').val().toUpperCase();
                    winnerId = data.winners[nhmc.config.USPSToState[stateShowing]];
                }
            } else {
                winnerId = data.winners[nhmc.config.USPSToState[stateShowing]];
    ...
    function parseHash() {
        ...
        if (typeof(hashMapView) != 'undefined' && $('#map_view').val() != hashMapView) {
            // Make sure it's a map view that's actually available to the user
            var selectedOption = $('.view_tab_option[href="#' + hashMapView + '"]').first();
            if (selectedOption.length != 0) {
                selectedOption.click();
            } else {
                nhmc.ctrl.hashParams({"map_view": $('#map_view').val()});
                sidebarInit();
                getMapData($('#map_view').val());
            }
        } else {
