...
nhmc.mapSpecificInit = function() {
    ...
    function liveDataInit(data) {
        ...
        for (var i = 0, length = data.breakdown.length; i < length; i++) {
            ...
            legendEntry.children('.candidate_color').css('background-color', data.colors[candidateId]);
            ...
            if (data.images && data.images[candidateId]) {
                legendEntry.children('.candidate_image').attr('src', data.images[candidateId]);
            }
