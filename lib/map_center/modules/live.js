...
nhmc.mapSpecificInit = function() {
    ...
    function liveDataInit(data) {
        ...
        for (var i = 0, length = data.breakdown.length; i < length; i++) {
            ...
            legendEntry.children('.candidate_votes').text(Math.round(candidateVotePercent) + '%');
            ...
            if (config.candidateImages[data.candidates[candidateId]]) {
                ...
                if (candidateImage.length) {
                    legendEntry.children('.candidate_image').replaceWith(candidateImage.clone());
                } else {
                    legendEntry.children('.candidate_image').attr('src', config.candidateImages[data.candidates[candidateId]]).addClass('candidate_image_' + candidateId).clone().appendTo('#legend_images');
                }
            }
        ...
        if (config.condenseCandidates && config.hoverExpandOther) {
            var otherElement = $('.candidate_small').filter(function(i) {
                var candidateName = $(this).children('.candidate_name');
                if (candidateName.children('.candidate_name_last').text() == 'Other' && candidateName.children('.candidate_name_first').text() == '') {
                    return true;
                } else {
                    return false;
                }
            ...
            // FIXME: This close button doesn't get added yet under any
            //  circumstances.
            otherTooltip.addClose = function() {
                $('#other_tooltip').prepend('<a href="#" id="other_tooltip_close" class="ui-icon ui-icon-close">Close</a>');
                $('#other_tooltip_close').click(function() {
            ...
            otherTooltip.unbindHover = function() {
                ...
                $('#other_tooltip_close').unbind('click');
            };
