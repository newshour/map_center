...
nhmc.mapSpecificInit = function() {
    ...
    function staticTypesInit() {
        ...
        if (staticCategories) {
        ...
        } else if (staticBreaks) {
            // Color each area appropriately.
            for (var areaId in staticAreas) {
                for (var i = 0, length = staticBreaks.length; i < length; i++) {
                    if (staticAreas[areaId] <= staticBreaks[i]) {
                        var areaFill = staticColors[i];
            ...
            for (var i = 0, length = staticBreaks.length; i < length; i++) {
                var breakText = '';
                if (i == 0) {
                    breakText = '&lt;' + staticBreaks[i].toFixed(1) + '%';
                } else if (i == length - 1) {
                    breakText = '&ge;' + staticBreaks[i - 1].toFixed(1);
                } else {
                    breakText = staticBreaks[i].toFixed(1) + '%&ndash;' + staticBreaks[i + 1].toFixed(1) + '%';
                }
                
                $('#static_legend').append('<li style="background-color: ' + staticColors[i] + ';"><span id="static_legend_' + i + '" class="county_type">' + breakText + '</span></li>');
            }
        }
    }
