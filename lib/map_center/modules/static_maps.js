...
nhmc.mapSpecificInit = function() {
    ...
    function fillCounties(i, clearFill) {
        ...
        var countyType = staticIds.from[i];
        var countyList = staticCounties[countyType];
        if (clearFill) {
            var countyFill = nhmc.config.styleColors['default'];
        } else {
            var countyFill = staticColors[countyType];
        }
    ...
    function staticTypesInit() {
        ...
        for (var i = 1; i <= 12; i++) {
            var countyType = staticIds.from[i];
            $('#static_legend').append('<li style="background-color: ' + staticColors[staticIds.from[i]] + ';"><a href="#" id="static_legend_' + i + '" class="county_type selected">' + countyType + '</a></li>');
            fillCounties(i);
        }
        ...
        $('#static_legend .county_type').click(function() {
            var countyTypeId = parseInt($(this).attr('id').substring(14));
