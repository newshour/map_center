...
nhmc.mapSpecificInit = function() {
    ...
    function staticTypesInit() {
        ...
        if (nhmcStatic.categories) {
            ...
            for (var i = 0, length = nhmcStatic.breaks.length; i < length; i++) {
                var breakText = '';
                if (i == 0) {
                    breakText = '&lt;' + nhmcStatic.breaks[i].toFixed(1) + '%';
                } else if (i == length - 1) {
                    breakText = '&ge;' + nhmcStatic.breaks[i - 1].toFixed(1);
                } else {
                    breakText = nhmcStatic.breaks[i].toFixed(1) + '%&ndash;' + nhmcStatic.breaks[i + 1].toFixed(1) + '%';
                }
    ...
    nhmc.tooltips.render = function() {
        ...
        $('#tooltip').remove();
        var thisFIPS = this.nhmcData.county_fips;
        $('body').append('<div id="tooltip">' + 
        '<h3>' + thisFIPS + '</h3>' + 
        '<p>' + nhmc.config.FIPSToCounty[thisFIPS][1] + ', ' + nhmc.config.FIPSToCounty[thisFIPS][0] + '</p>' + 
        '</div>');
    };
    nhmc.tooltips.init();
