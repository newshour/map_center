...
nhmc.config.cityRadii = {
    'default': 4
};
nhmc.config.cityLabelSize = 18;  // font size of city label text in pixels
...
nhmc.ctrl.addCity = function(cityName, cityData) {
    ...
    var cityMarkerData = {
        cx: cityData[0][0],
        cy: cityData[0][1],
        r: nhmc.config.cityRadii[cityData[1]] || nhmc.config.cityRadii['default'],
        fontSize: nhmc.config.cityLabelSize
    };
    ...
    // Add the city's name.
    var cityLabel = nhmc.surface.createText({
        x: cityMarkerData.cx + (2 * cityMarkerData.r),
        y: cityMarkerData.cy + (cityMarkerData.fontSize / 2) - 1,
        text: cityName.toUpperCase()
    }).setFont({
        family: 'Arial',
        size: cityMarkerData.fontSize + 'px',
        weight: 'bold'
    }).setFill({
        r: 0,
        g: 0,
    ...
    if ((cityLabel.getShape().x + cityLabel.getTextWidth() + cityMarkerData.r) > nhmc.config.defaultDimensions.width) {
        // If so, move it to the left of the marker instead.
        cityLabel.setShape({
            x: cityMarkerData.cx - (2 * cityMarkerData.r),
            y: cityMarkerData.cy + cityMarkerData.r,
            text: cityName.toUpperCase(),
            align: 'end'
        });
    }
    ...
    if (cityLabel.getShape().align != 'end') {
        var cityBGData = {
            r: (cityMarkerData.fontSize + (2 * cityMarkerData.r)) / 2,
            arcsSweep: 0,
            labelN: [cityLabel.getShape().x + cityLabel.getTextWidth() + 2, cityLabel.getShape().y - cityMarkerData.fontSize - cityMarkerData.r],
            labelS: [cityLabel.getShape().x + cityLabel.getTextWidth() + 2, cityLabel.getShape().y + cityMarkerData.r],
            markerN: [cityMarkerData.cx, cityLabel.getShape().y - cityMarkerData.fontSize - cityMarkerData.r],
            markerS: [cityMarkerData.cx, cityLabel.getShape().y + cityMarkerData.r]
        };
    } else {
        var cityBGData = {
            r: (cityMarkerData.fontSize + (2 * cityMarkerData.r)) / 2,
    ...
    var cityBackground = nhmc.surface.createPath()
        // starting from directly south of the marker,
        .moveTo(cityBGData.markerS[0], cityBGData.markerS[1])
        // continuing to south of the other end of the label,
        .lineTo(cityBGData.labelS[0], cityBGData.labelS[1])
        // continuing up to north of the same end of the label,
        .lineTo(cityBGData.labelN[0], cityBGData.labelN[1])
        // going north of the marker,
        .lineTo(cityBGData.markerN[0], cityBGData.markerN[1])
        // arcing back to where we started,
        .arcTo(cityBGData.r, cityBGData.r, 90, 0, cityBGData.arcsSweep, cityBGData.markerS[0], cityBGData.markerS[1])
        // and finally closing the path.
        .closePath()
        .setFill({
            r: 255,
            g: 255,
    ...
    if (cityGroup.getNode()) {$(cityGroup.getNode()).attr('class', 'city_label');}
    else {
        cityMarker.setFill({
            r: 0,
            g: 0,
...
nhmc.tooltips.addClose = function() {
    $('<a href="#" id="tooltip_close" class="ui-icon ui-icon-close">Close</a>').prependTo('#tooltip').click(function() {
        nhmc.tooltips.destroy();
