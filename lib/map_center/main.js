...
nhmc.ctrl.zoomToState = function(state) {
    ...
    if (state != 'us_all' && state != 'us_geo' && state != 'us_counties' && state != 'conus') {
        ...
        if (!$.isEmptyObject(nhmc.geo.usGeo[state].countyPaths)) {
            ...
            var intervalId;
            var step = 0;
            var totalSteps = nhmc.config.countySteps.state.steps;
            ...
            intervalId = window.setInterval(reloadMoreOfState, nhmc.config.countySteps.state.span / totalSteps);
        } else {
            $('#loading').show();
            ...
            function loadMoreOfState() {
                if (step < totalSteps) {
                    ...
                    var endBreak = Math.floor((step + 1) * keys.length / totalSteps);
                    for (var j=startBreak; j<endBreak; j++) {
                        nhmc.geo.usGeo[state].countyPaths[keys[j]] = nhmc.R.path(nhmc.geo.usGeo[state].counties[keys[j]]);
            ...
            intervalId = window.setInterval(loadMoreOfState, nhmc.config.countySteps.state.span / totalSteps);
        }
        ...
        if (!$.isEmptyObject(nhmc.geo.countyGeo)) {
            ...
            var intervalId;
            var step = 0;
            var totalSteps = nhmc.config.countySteps.national.steps;
            ...
            intervalId = window.setInterval(reloadMoreCounties, nhmc.config.countySteps.national.span / totalSteps);
        } else {
            $('#loading').show();
            ...
            function loadMoreCounties() {
                if (step < totalSteps) {
                    ...
                    var endBreak = Math.floor((step + 1) * keys.length / totalSteps);
                    for (var i=startBreak; i<endBreak; i++) {
                        nhmc.geo.countyGeo[keys[i]] = nhmc.R.path(nhmc.geo.allCounties[keys[i]]);
            ...
            intervalId = window.setInterval(loadMoreCounties, nhmc.config.countySteps.national.span / totalSteps);
        }
    }
};
...
nhmc.mapCenterInit = function() {
    nhmc.R = Raphael('map', 768, 495);
    for (var i in nhmc.geo.usGeo) {
        nhmc.geo.usGeo[i].statePath = nhmc.R.path(nhmc.geo.usGeo[i].state);
