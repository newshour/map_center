            ...
            function loadMoreOfState() {
                if (step < totalSteps) {
                    ...
                    for (var cityName in nhmc.geo.usGeo[state].cities) {
                        var cityData = nhmc.geo.usGeo[state].cities[cityName];
                        ...
                        if (cityLabel.getShape().align != 'end') {
                            var cityBGData = {
                                r: 2 * cityMarkerData.r,
                                arcsSweep: 0,
                                labelN: [cityLabel.getShape().x + cityLabel.getTextWidth(), cityMarkerData.cy - (2 * cityMarkerData.r)],
                                labelS: [cityLabel.getShape().x + cityLabel.getTextWidth(), cityMarkerData.cy + (2 * cityMarkerData.r)],
                                markerN: [cityMarkerData.cx, cityMarkerData.cy - (2 * cityMarkerData.r)],
                                markerS: [cityMarkerData.cx, cityMarkerData.cy + (2 * cityMarkerData.r)]
                            };
                        } else {
                            var cityBGData = {
                                r: 2 * cityMarkerData.r,
                        ...
                        var cityBackground = nhmc.surface.createPath()
                            ...
                            // arcing up to north of the same end of the label,
                            .arcTo(cityBGData.r, cityBGData.r, 270, 0, cityBGData.arcsSweep, cityBGData.labelN[0], cityBGData.labelN[1])
                        ...
                        nhmc.geo.usGeo[state].cityPaths[cityName] = cityGroup;
                        if (scalingNecessary) {
                            nhmc.geo.usGeo[state].cityPaths[cityName].setTransform(scaleMatrix);
                        }
                    }
